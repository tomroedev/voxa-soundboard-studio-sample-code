...
This is a small slice of the codebase, not the full project. These are excerpts (full classes) with dependencies that live in the private repo, so imports will not resolve if the project is built. 
...
Some test files are included. These run automatically on Firebase Test Lab via GitHub actions whenever the version code is increased (i.e. when I'm creating a new build for the Play Store). They also run automatically every 8 hours so that errors in code can be picked up quickly.
